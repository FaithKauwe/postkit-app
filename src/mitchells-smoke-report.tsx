...
/** For React components: compare a substring in server-rendered HTML (same pass/fail table UX). */
function checkContains(label: string, input: string, needle: string, element: ReactElement): TestResult {
  const html = renderToStaticMarkup(element)
  return check(label, input, needle, html.includes(needle) ? needle : html.replace(/\s+/g, ' ').slice(0, 200))
}

const thStyle: CSSProperties = { textAlign: 'left', padding: '4px 8px', borderBottom: '2px solid #ccc' }
const tdStyle: CSSProperties = { padding: '4px 8px' }
...
function TestPackages() {
  const smokeStorageKey = '__postkit_mitchell_smoke_storage__'
  savePosts(smokeStorageKey, [])
  ...
  // --- postkit-validation-library (meets default min title 3, min body 20) ---
  const validValidationPost: Post = {
    id: 'val-1',
    title: 'Valid Post Title',
    body: 'This body is definitely long enough.',
    author: 'Casey',
    tags: ['news'],
    category: 'General',
    status: 'review',
  ...
  const validateTitleTests: TestResult[] = [
    check('empty title', '""', 'false', validateTitle('').valid.toString()),
    check('short title', '"Hi"', 'false', validateTitle('Hi').valid.toString()),
    check('ok title', '"Hello"', 'true', validateTitle('Hello').valid.toString()),
    check('first issue code empty', '""', 'REQUIRED', validateTitle('').issues[0]?.code ?? ''),
  ]

  const validateBodyTests: TestResult[] = [
    check('empty body', '""', 'false', validateBody('').valid.toString()),
    check('short body', '"too short"', 'false', validateBody('too short').valid.toString()),
    check('long enough', '20+ chars', 'true', validateBody('x'.repeat(20)).valid.toString()),
    check('first issue empty', '""', 'REQUIRED', validateBody('').issues[0]?.code ?? ''),
  ]

  const validateStatusTests: TestResult[] = [
    check('draft', '"draft"', 'true', validateStatus('draft').valid.toString()),
    check('review', '"review"', 'true', validateStatus('review').valid.toString()),
    check('published', '"published"', 'true', validateStatus('published').valid.toString()),
    check('invalid', '"live"', 'false', validateStatus('live').valid.toString()),
    check('invalid code', '"live"', 'INVALID_STATUS', validateStatus('live').issues[0]?.code ?? ''),
  ]

  const validatePostTests: TestResult[] = [
    check('valid post', 'full fields', 'true', validatePost(validValidationPost).valid.toString()),
    check('missing title', 'title=""', 'false', validatePost({ ...validValidationPost, title: '' }).valid.toString()),
    check('isPostValid shortcut', 'valid', 'true', isPostValid(validValidationPost).toString()),
    check('isPostValid invalid status', 'status=x', 'false', isPostValid({ ...validValidationPost, status: 'x' as unknown as Post['status'] }).toString()),
    check('errors codes join', 'bad status', 'INVALID_STATUS', getPostValidationErrors({ ...validValidationPost, status: 'x' as unknown as Post['status'] }).map(i => i.code).join(', ')),
  ]

  // --- postkit-storage-lib ---
  const storageLoadPostsTests: TestResult[] = [
    check('loadPosts after clear', 'length', '0', String(loadPosts(smokeStorageKey).length)),
  ]

  savePosts(smokeStorageKey, [validValidationPost])

  const storageSaveLoadTests: TestResult[] = [
    check('loadPosts first id', 'after save', validValidationPost.id, loadPosts(smokeStorageKey)[0]?.id ?? ''),
  ]

  const storageImportExportTests: TestResult[] = [
    check('export includes id', 'json', 'true', exportPosts([validValidationPost]).includes(validValidationPost.id) ? 'true' : 'false'),
    check('importPosts roundtrip title', 'export→import', validValidationPost.title, importPosts(exportPosts([validValidationPost]))[0]?.title ?? ''),
    check('importPosts invalid object', '"{}"', 'throws', tryCatch(() => { importPosts('{}'); return 'ok' })),
    check('importPosts invalid json', '"BAD"', 'throws', tryCatch(() => { importPosts('BAD'); return 'ok' })),
  ]

  savePosts(smokeStorageKey, [])

  const storageClearTests: TestResult[] = [
    check('loadPosts after final clear', 'length', '0', String(loadPosts(smokeStorageKey).length)),
  ]

  // --- postkit-ui-component-library (Status uses draft | published | archived) ---
  const statusBadgeTests: TestResult[] = [
    checkContains('draft label', 'status=draft', 'Draft', <StatusBadge status="draft" />),
    checkContains('published label', 'status=published', 'Published', <StatusBadge status="published" />),
    checkContains('archived label', 'status=archived', 'Archived', <StatusBadge status="archived" />),
    checkContains('test id', 'data-testid', 'data-testid="status-badge"', <StatusBadge status="draft" />),
  ]

  const searchInputTests: TestResult[] = [
    checkContains('renders input', 'value=""', 'type="text"', <SearchInput value="" onChange={() => {}} aria-label="Search posts" />),
    checkContains('aria-label', 'Search posts', 'aria-label="Search posts"', <SearchInput value="" onChange={() => {}} aria-label="Search posts" />),
    checkContains('placeholder', 'ph', 'placeholder="Find…"', <SearchInput value="" onChange={() => {}} placeholder="Find…" aria-label="Search" />),
  ]

  const postCardTests: TestResult[] = [
    checkContains('title in markup', 'Card Title', 'Card Title', <PostCard title="Card Title" author="Pat" status="draft" />),
    checkContains('author line', 'by Pat', 'by Pat', <PostCard title="Card Title" author="Pat" status="draft" />),
    checkContains('published badge', 'status', 'Published', <PostCard title="Card Title" author="Pat" status="published" />),
  ]
  ...
  return (
    <div style={{ padding: 24, fontFamily: 'sans-serif' }}>
      ...
      <h1>postkit-validation-library — test results</h1>
      <Table title="validateTitle" rows={validateTitleTests} />
      <Table title="validateBody" rows={validateBodyTests} />
      <Table title="validateStatus" rows={validateStatusTests} />
      <Table title="validatePost / isPostValid / getPostValidationErrors" rows={validatePostTests} />

      <h1>postkit-storage-lib — test results</h1>
      <Table
        title="savePosts / loadPosts / exportPosts / importPosts"
        rows={[
          ...storageLoadPostsTests,
          ...storageSaveLoadTests,
          ...storageImportExportTests,
          ...storageClearTests,
        ]}
      />

      <h1>postkit-ui-component-library — test results</h1>
      <Table title="StatusBadge" rows={statusBadgeTests} />
      <Table title="SearchInput" rows={searchInputTests} />
      <Table title="PostCard" rows={postCardTests} />
    </div>
  )
}
